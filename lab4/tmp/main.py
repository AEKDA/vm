...
def sort_diagram(A, B, n):
    ...
    print(
        "Невозможно составить матрицу c диагональным преобладанием!!! Возможномть несходимость Слау"
    )
    return A, B
...
def sort_diag_arr(a, b):
    n = len(a)
    for i in range(n):
        ind, a[i:] = zip(*sorted(enumerate(a[i:]), key=lambda x: -x[1][i]))
...
def iterational_method(C, D, result_matrix, accuracy, n, iteration_counter, A, B):
    ...
    max_deviation = max(count_residual(new_answers, A, B, n))
    new_answers.append(max(count_residual(new_answers, A, B, n)))
    # print(new_answers)
    ...
    if max_deviation < accuracy:
        ...
        if iteration_counter == 100:
            ...
            return result_matrix
        return iterational_method(
            C, D, result_matrix, accuracy, n, iteration_counter + 1, A, B
        )


def count_residual(answer_matrix, A, B, n):
    residual_arr = []
    for i in range(n):
        calc_ans = 0
        for j in range(n):
